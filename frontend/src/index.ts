...
let canvas = new Canvas(html.getContext("2d")!, size, {x: 20, y: 20})
...
function draw(message: Infos) {
  ...
  if(player){
    canvas.setPlayerPos(player.positions[player.positions.length-1])
  }
  ...
  // grid
  canvas.drawGrid()
  canvas.drawBondaries(message.size)
  ...
  if(player){
    ...
    if(player.state instanceof Object && "Waiting" in player.state && player.state.Waiting%2 == 0) {
      canvas.drawTextCenter((player.state.Waiting/2).toString(), "white", 70);
    }
  }
